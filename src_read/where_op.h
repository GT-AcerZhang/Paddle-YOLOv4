...
class WhereKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    auto* out =       context.Output<framework::Tensor>("Out");   // [M, ?]  M是符合条件的元素数，?是坐标的长度。

    const bool* cond_data = condition->data<bool>();   // 调用data()函数获取指针
    const T* x_data = X->data<T>();   // 调用data()函数获取指针
    const T* y_data = Y->data<T>();   // 调用data()函数获取指针
    T* out_data = out->mutable_data<T>(context.GetPlace());   // 调用mutable_data()函数获取指针，可以指定形状

    auto x_numel = X->numel();   // 调用numel()函数获取X的元素个数
